package com.vkc.strabo.activity.redeem
...
import com.vkc.strabo.activity.redeem.adapter.RedeemAdapter
...
class RedeemHistoryActivity : Activity(), VKCUrlConstants {
    ...
    private fun initUI() {
        ...
        listViewRedeem!!.onItemClickListener = OnItemClickListener { parent, view, position, id ->
            val dialog: DialogData = mContext?.let { DialogData(it, position) }!!
            dialog.show()
        }
        history
                                ...
                                try {
                                    ...
                                    if (status.equals("Success", ignoreCase = true)) {
                                        ...
                                        if (dataArray.length() > 0) {
                                            ...
                                            val adapter =
                                                mContext?.let { RedeemAdapter(it, listGifts!!) }
